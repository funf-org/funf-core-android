...
public class HashUtil {
	...
	private static String oneWayHashAndRSA(Context context, String msg){
		...
		jsonEncMsg.put(HashingType.ONE_WAY_HASH.name(), oneWayHashString(msg));
		jsonEncMsg.put(HashingType.RSA_ENC.name(), RSAEncode.encodeStringRSA(context, msg));
	...
	public static String hashString(Context context, String msg) {
		return hashString(context, msg, HashingType.INTERMEDIATE_HASH_ENC);
	}
		
	public static String hashString(Context context, String msg, HashingType hashingType){
		if (hashingType == HashingType.ONE_WAY_HASH){
			try{ 
				return 
					(new JSONObject()).put(HashingType.ONE_WAY_HASH.name(), oneWayHashString(msg)).toString();
			}catch(JSONException e){
...
// by Wei Pan
	public static String formatPhoneNumber(String numberString){
		...
		if (i <= 10)
			...
			return numberString.substring(i-10); // only look at the last 10 numbers
